...
public function Pbbq(){
        if(!IS_LOGIN)
            return $this->message('请登录');

       
        $name = htmlspecialchars(trim(X("post.name")));
        ...
        if($f and $f['only']==1)
            return $this->message("校区已经存在");
        if($n2)
            return $this->message("校区别名已经存在");
        ...
        return $this->message('提交成功 返回上一页刷新页面！',true);
     
    }
//修改表白墙信息
public function upbbq(){
        if(!IS_LOGIN)
            return $this->message('请登录');
        ...
        if(empty($name) or empty($name1))
            return $this->message("内容不能为空");
        if(mb_strlen($name) > 20 or mb_strlen($name1) > 20)
        ...
        $Thread->update(array(
            ...
            ),array("forumg"=>NOW_UID));
        return $this->message('修改成功 返回上一页刷新！',true);
